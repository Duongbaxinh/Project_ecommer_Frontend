...
export const fetchDetailProduct = async (id_product) => {
  try {
    const product = await axios.get(
      `http://127.0.0.1:8000/api/v1/product/${id_product}/`
    );
    const images = await axios.get(
      `http://127.0.0.1:8000/api/v1/product/${id_product}/images/`
    );
    const sameProduct = await axios.get(
      `http://127.0.0.1:8000/api/v1/product/${product.data.data["category_id"]}/category/`
    );
...
export const checkout = async ({ id_cart, type }) => {
  try {
    ...
    return await axios.put(
      `http://127.0.0.1:8000/api/v1/cart/${id_cart}/`,
      { type: type },
...
export const handleDeleteCartService = async (idCart) => {
  try {
    ...
    const { data } = await axios.delete(
      `http://127.0.0.1:8000/api/v1/cart/${idCart}/`,
      { headers: { Authorization: `Bearer ${accessToken}` } }
    );
    return data;
  } catch (error) {}
...
export const getAllOrder = async () => {
  try {
    const accessToken = JSON.parse(localStorage.getItem("accessToken"));
    const { data } = await axios.get("http://127.0.0.1:8000/api/v1/orders/", {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    return data;
  } catch (error) {}
